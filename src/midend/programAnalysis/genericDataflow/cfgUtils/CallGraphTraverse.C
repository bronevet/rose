...
CGFunction::iterator::iterator(const CGFunction* const func, direction dir)
{
  ...
  //Dbg::dbg << "CGFunction::iterator::iterator(func="<<func->str()<<", direction="<<(dir==fw?"fw":"bw")<<endl;
...
const CGFunction* CGFunction::iterator::getTarget(std::set<CGFunction> &functions)
{
  ...
  //Dbg::dbg << "isSgFunctionDeclaration(target->get_SgNode())->get_file_info()->isCompilerGenerated()="<<isSgFunctionDeclaration(target->get_SgNode())->get_file_info()->isCompilerGenerated()<<endl;
  ...
  //Dbg::dbg << "getTarget returning NULL since couldn't find target function"<<endl;
...
{
  //Dbg::dbg << "Function::iterator::advanceToNextValidPoint() #edges="<<edges.size()<<" last edge="<<(ite == edges.end())<<endl;
  ...
  while(!finished && itn != func->cgNodes.end())
  {
    ...
    if(ite == edges.end())
    {
      /*Dbg::dbg << "Function::iterator::advanceToNextValidPoint() while(), last="<<(itn == func->cgNodes.end())<<endl;
      Dbg::indent ind;*/
    ...
    else
    {
      SgGraphNode* target = (dir == fw ? (*ite)->get_to() : (*ite)->get_from());
        ...
        /*{
          ...
          if(target) {
            Function func(isSgFunctionDeclaration(target->get_SgNode()));
            Dbg::dbg << "    advancing within CG Node target="<<func.str()<<", visited="<<(visitedCGNodes.find(target) != visitedCGNodes.end())<<endl;
          } else
            Dbg::dbg << "    advancing within CG Node target=NULL"<<", visited="<<(visitedCGNodes.find(target) != visitedCGNodes.end())<<endl;
        }*/
