...
namespace dataflow {
int AbstractObjectMapDebugLevel=0;

bool MayEqualFunctor::mayEqual(AbstractObjectPtr objPtr1, AbstractObjectPtr objPtr2, PartEdgePtr pedge, Composer* comp, ComposedAnalysis* analysis) {
  ...
  // else
  return objPtr1->mayEqual(objPtr2, pedge, comp, analysis) || objPtr2->mayEqual(objPtr1, pedge, comp, analysis);
}

bool MustEqualFunctor::mustEqual(AbstractObjectPtr objPtr1, AbstractObjectPtr objPtr2, PartEdgePtr pedge, Composer* comp, ComposedAnalysis* analysis) {
  ROSE_ASSERT(objPtr1); ROSE_ASSERT(objPtr2);
...
Dbg::dbg << "&nbsp;&nbsp;&nbsp;&nbsp;"<<objPtr2->str("        ")<<endl;*/
  ...
  return objPtr1->mustEqual(objPtr2, pedge, comp, analysis);
}
...
bool AbstractObjectMap::insert(AbstractObjectPtr o, LatticePtr lattice) {
  Dbg::region reg(AbstractObjectMapDebugLevel, 1, Dbg::region::midLevel, "AbstractObjectMap::insert()");
  if(AbstractObjectMapDebugLevel>=1) {
    Dbg::dbg << "&nbsp;&nbsp;&nbsp;&nbsp;o="<<o->strp(latPEdge, "")<<" lattice="<<lattice->str("&nbsp;&nbsp;&nbsp;&nbsp;")<<" mapIsFull="<<mapIsFull<<endl;
    Dbg::dbg << "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"<<str("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;")<<endl;
  }
  ...
  for(it = items.begin(); it != items.end(); i++) {
    ...
    if(AbstractObjectMapDebugLevel>=2) Dbg::dbg << "&nbsp;&nbsp;&nbsp;&nbsp;keyElement="<<keyElement->str("            ")<<" mustEqual(o, keyElement, latPEdge)="<<mustEqual(o, keyElement, latPEdge, comp, analysis)<<" insertDone="<<insertDone<<" mustEqualSeen="<<mustEqualSeen<<endl;
    ...
    if(insertDone) {
      ...
      if(mustEqual(o, keyElement, latPEdge, comp, analysis)) {
        //if(mustEqualSeen) {
          items.erase(it++);
        //} else 
        //  it++;
    ...
    if(mustEqual(o, keyElement, latPEdge, comp, analysis)) {
      if(AbstractObjectMapDebugLevel==1) Dbg::dbg << "&nbsp;&nbsp;&nbsp;&nbsp;keyElement="<<keyElement->str("            ")<<" mustEqual(o, keyElement, latPEdge)="<<mustEqual(o, keyElement, latPEdge, comp, analysis)<<" insertDone="<<insertDone<<" mustEqualSeen="<<mustEqualSeen<<endl;
      if(AbstractObjectMapDebugLevel>=1) {
    ...
    // If the new element and the original actually denote the same set
      ...
      //Dbg::dbg << "o="<<o->str()<<" <b>equalSet</b> "<<keyElement<<" keyElement="<<keyElement->str()<<endl;
    ...
    } else if(mayEqual(o, keyElement, latPEdge, comp, analysis)) {
      //Dbg::dbg << "o="<<o->str()<<" <b>mayEqual</b> "<<keyElement<<" keyElement="<<keyElement->str()<<endl;
      items.push_front(MapElement(o, lattice));
      retVal = true;
      insertedLattice = lattice;
      it++;
...
// If this map corresponds to all possible mappings, all removals are redundant
  ...
  for (list<MapElement>::iterator it = items.begin();
    ...
    if (mustEqual(abstractObjectPtr, keyElement, latPEdge, comp, analysis)) {
      it = items.erase(it);
      return true;
    }
  }
...
Lattice* AbstractObjectMap::remapML(const std::set<pair<MemLocObjectPtr, MemLocObjectPtr> >& ml2ml, PartEdgePtr newPEdge)
{
  ...
  if(AbstractObjectMapDebugLevel>=1) {
    ...
    for(std::set<pair<MemLocObjectPtr, MemLocObjectPtr> >::const_iterator m=ml2ml.begin(); m!=ml2ml.end(); m++)
      ...
      if(!m->first->isLive(latPEdge, comp, analysis) || (m->second && !m->second->isLive(newPEdge, comp, analysis)))
        Dbg::dbg << "<b>AbstractObjectMap::remapML() WARNING: Skipping dead ml2ml mapping "<<m->first->strp(latPEdge)<<"(live="<<m->first->isLive(latPEdge, comp, analysis)<<") => "<<(m->second ? m->second->strp(newPEdge) : "NULL")<<"(live="<<(m->second ? m->second->isLive(newPEdge, comp, analysis) : -1)<<")"<<endl
                 << "&nbsp;&nbsp;&nbsp;&nbsp;latPEdge=["<<latPEdge->str()<<"]"<<endl
                 << "&nbsp;&nbsp;&nbsp;&nbsp;newPEdge=["<<newPEdge->str()<<"]</b>"<<endl;
  }
  
  AbstractObjectMap* newM = new AbstractObjectMap(equalFunctor, defaultLat, newPEdge, comp, analysis);
  ...
  for(std::list<MapElement>::iterator i=items.begin(); i!=items.end(); i++) {
    ...
    for(std::set<pair<MemLocObjectPtr, MemLocObjectPtr> >::const_iterator m=ml2ml.begin(); m!=ml2ml.end(); m++, mIdx++) {
      ...
      if(i->first->mustEqual(m->first, latPEdge, comp, analysis)) {
      ...
      } else if(i->first->mayEqual(m->first, latPEdge, comp, analysis)) {
        // Insert the corresponding value in ml2ml if it is not NULL
    ...
    if(!existsMustEqual) {
      ...
      if(!i->first->isLive(newPEdge, comp, analysis)) continue;
  ...
  for(std::set<pair<MemLocObjectPtr, MemLocObjectPtr> >::iterator m=ml2ml.begin(); m!=ml2ml.end(); m++, mIdx++) {
    // If either the key or the value of this mapping is dead within its respective part, skip it
    if(!m->first->isLive(latPEdge, comp, analysis) || !m->second->isLive(newPEdge, comp, analysis)) continue;
...
{
  Dbg::region reg(AbstractObjectMapDebugLevel, 2, Dbg::region::midLevel, "AbstractObjectMap::meetUpdate()");
  ...
  try {
    ...
    /*if(AbstractObjectMapDebugLevel>=1) {
      Dbg::dbg << "thisMustEq2thatMustEq="<<endl;
      Dbg::indent ind(1, 1);
      for(list<pair<list<MapElement>::iterator, pair<list<MapElement>::iterator, int> > >::iterator it=thisMustEq2thatMustEq.begin();
        ...
        Dbg::dbg << (it->first)->first->str() << " => " << (it->second).first->second->str() << endl;
      }
    }*/
    
    if(AbstractObjectMapDebugLevel>=2) Dbg::dbg << "thisMustEq2thatMustEq="<<endl;
    Dbg::indent ind(AbstractObjectMapDebugLevel, 2);
    ...
    for(list<pair<list<MapElement>::iterator, pair<list<MapElement>::iterator, int> > >::iterator meIt=thisMustEq2thatMustEq.begin();
      ...
      if(AbstractObjectMapDebugLevel>=2) Dbg::dbg << "mustEqual mapping "<<meIt->second.second<<": "<<(meIt->first)->first->str() << " /// " << (meIt->second).first->first->str() << " => " << (meIt->second).first->second->str() << ", thatIdx="<<thatIdx<<endl;
      ...
      if(meIt->second.second >= thatIdx) {
        for(; thatIt!=meIt->second.first; thatIt++, thatIdx++, thatMEIt++) {
          ...
          if(!(*thatMEIt)) {
            if(AbstractObjectMapDebugLevel>=2) Dbg::dbg << "Inserting at meIt->first="<<(meIt->first)->first->str()<<" mapping "<<thatIt->first->str()<<" ==> "<<thatIt->second->str()<<endl;
    ...
    for(; thatIt!=that->items.end(); thatIt++) {
      if(AbstractObjectMapDebugLevel>=2) Dbg::dbg << "Pushing end "<<thatIt->first->str()<<" ==> "<<thatIt->second->str()<<endl;
      // NOTE: we do not currently update the part field in the lattice thatIt->second
      //       to refer to this->latPEdge. Perhaps we should make a copy of it and update it.
    ...
    //Dbg::dbg << "Before mustEq compression "<<str()<<endl;
    ...
    //Dbg::dbg << "Before dead compression "<<str()<<endl;
    compressDead();
    
    //Dbg::dbg << "Final "<<str()<<endl;
  } catch (bad_cast & bc) { 
    ROSE_ASSERT(false);
  }
  return modified;
...
bool AbstractObjectMap::compressMustEq()
{
  ...
  for(list<MapElement>::iterator x = items.begin(); x != items.end(); x++, xIdx++) {
    ...
    for(; y != items.end(); yIdx++) {
      ...
      if(mustEqual(x->first, y->first, latPEdge, comp, analysis)) {
